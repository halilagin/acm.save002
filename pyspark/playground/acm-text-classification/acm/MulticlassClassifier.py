...
class MulticlassLogisticRegressionModelTrainer(object):
    ...
    def start(self,q,parentEnv, configJsonStr):
        ...
        #set config
        config = DictionaryAsNestedObjectSerializer(json.loads(configJsonStr))
        trainDataFiles=os.path.join(config.acm.models.classification.inputDir, "*.csv")
        ...
        outputDir=config.acm.models.classification.outputDir
        zipDir=config.acm.models.classification.zipDir
        modelSavePolicy=config.acm.models.classification.modelSavePolicy
        ...
        if modelSavePolicy=="mostRecentOne":
            ...
            if not os.path.exists(zipDir):
                os.mkdir(zipDir)
            modelFileName = time_ms+".logistic-regression.model" 
            modelOutputPath = os.path.join(outputDir, modelFileName)
            lrModel.save(modelOutputPath)
            zer=ZipFileWriter()
...
class MulticlassClassifierExecutor(object):
    ...
    def start(self):
        pass
        fs = [self.lrModelTrainer, self.lrModelTrainer,] #functions
        ps=[] #processes
        qs=[] # queues
