...
from mlstreaming.base import SparkLogisticWrapperBase
from mlstreaming.util import loadrecent


class SparkLogisticWrapper(SparkLogisticWrapperBase):
        ...
        for i in range(1, self.nbatches):
            ...
            if len(model) == self.ncenters:
                ...
                if (lgn is not None) & (i > 5):
                    ...
                    if self.ndims == 2:
                        if viz is None:
                            viz = lgn.scatterstreaming(pts[:, 0], pts[:, 1], labels=clrs, size=s)
                        else:
                    ...
                    # line plot for one dimension
                        ...
                        if viz is None:
                            viz = lgn.linestreaming(pts, labels=clrs, size=s/2)
                        else:
