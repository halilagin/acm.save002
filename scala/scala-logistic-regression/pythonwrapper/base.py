...
class SparkLogisticWrapperBase(object):
    ...
    def setup(self, path, overwrite=False):
        ...
        if os.path.isdir(path):
            if overwrite:
                pass
                files = glob.glob(os.path.join(path,'input/*'))
                for f in files:
                    os.remove(f)
                files = glob.glob(os.path.join(path,'output/*'))
                for f in files:
                    os.remove(f)
                #shutil.rmtree(path)
                #os.mkdir(path)
    ...
    def writepoints(self, pts, i):
        ...
        f = open(os.path.join(self.datain, 'batch%g.txt' % i), 'w')
